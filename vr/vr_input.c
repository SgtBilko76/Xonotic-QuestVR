...
static float triggerValue[2];
static float gripValue[2];
...
void IN_VR_Vibrate( float duration, int channel, float intensity ) {
	if (intensity > 0) {
		...
		memset(&hapticActionInfo, 0, sizeof(hapticActionInfo));
...
void IN_VRInit( engine_t *engine ) {
	...
	memset(&suggestedBindings, 0, sizeof(suggestedBindings));
...
void IN_VRInputFrame( engine_t* engine ) {
	...
	memset(&activeActionSet, 0, sizeof(activeActionSet));
	...
	gripValue[0] = GetActionStateFloat(gripLeftAction).currentState;
	triggerValue[0] = GetActionStateBoolean(indexLeftAction).currentState ? 1.0f : 0.0f;
	if (gripValue[0] > 0.5f) lButtons |= ovrButton_GripTrigger;
	...
	gripValue[1] = GetActionStateFloat(gripRightAction).currentState;
	triggerValue[1] = GetActionStateBoolean(indexRightAction).currentState ? 1.0f : 0.0f;
	if (gripValue[1] > 0.5f) rButtons |= ovrButton_GripTrigger;
...
float IN_VRGetTrigger( int controllerIndex ) {
	return (controllerIndex >= 0 && controllerIndex < 2) ? triggerValue[controllerIndex] : 0.0f;
}

float IN_VRGetGrip( int controllerIndex ) {
	return (controllerIndex >= 0 && controllerIndex < 2) ? gripValue[controllerIndex] : 0.0f;
}
