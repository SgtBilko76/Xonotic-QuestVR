...
/* QuakeQuest's QuatToYawPitchRoll(): optional local rotation (degrees) applied before extracting Quake angles */
XrVector3f XrQuaternionf_ToQuakeAngles(const XrQuaternionf q, float rotPitch, float rotYaw, float rotRoll) {
	ovrMatrix4f mat = ovrMatrix4f_CreateFromQuaternion(&q);

	if (rotPitch != 0.0f || rotYaw != 0.0f || rotRoll != 0.0f)
	{
		ovrMatrix4f rot = ovrMatrix4f_CreateRotation(ToRadians(rotPitch), ToRadians(rotYaw), ToRadians(rotRoll));
	...
	XrVector4f forwardInVRSpace = XrVector4f_MultiplyMatrix4f(&mat.M[0][0], &v1);
	XrVector4f rightInVRSpace = XrVector4f_MultiplyMatrix4f(&mat.M[0][0], &v2);
	XrVector4f upInVRSpace = XrVector4f_MultiplyMatrix4f(&mat.M[0][0], &v3);

	XrVector3f forward = {-forwardInVRSpace.z, -forwardInVRSpace.x, forwardInVRSpace.y};
	XrVector3f right = {-rightInVRSpace.z, -rightInVRSpace.x, rightInVRSpace.y};
	XrVector3f up = {-upInVRSpace.z, -upInVRSpace.x, upInVRSpace.y};

	return XrVector3f_GetAnglesFromVectors(XrVector3f_Normalized(forward), XrVector3f_Normalized(right), XrVector3f_Normalized(up));
}
